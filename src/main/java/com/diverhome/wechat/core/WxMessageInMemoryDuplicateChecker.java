...
public class WxMessageInMemoryDuplicateChecker implements
	 ...
	 * 后台清理线程是否已经开启
	 */
	private final AtomicBoolean backgroundProcessStarted = new AtomicBoolean(
			false);
			...
			public void run() {
				try {
					while (true) {
						...
						for (Map.Entry<String, Long> entry : msgId2Timestamp
								.entrySet()) {
	...
	public boolean isDuplicate(String messageId) {
		...
		Long timestamp = msgId2Timestamp.putIfAbsent(messageId, System
				.currentTimeMillis());
		if (timestamp == null) {
			// 第一次接收到这个消息
			return false;
