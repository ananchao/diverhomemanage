...
<body class="gray-bg">
    ...
    <script>
        $(document).ready(function () {
            // 获取所有教练信息
            $.get("${ctx}/manage/getEmployeeList", {type: '0'}).then(function(res){
            	if(res.status==1){
            		// 成功
            		$(res.data).each(function(index, item){
            			$('<div class="col-sm-4"><div class="contact-box"><div class="col-sm-4"><div class="text-center"><img alt="image" class="img-circle m-t-xs img-responsive" src="${ctx}/static/img/a3.jpg"><div class="m-t-xs font-bold">'
            					...
            					+item.name+'</strong></h3><p><strong>PADI:'
            					+item.serial_number+'</strong></p><address>Age:'
            					+item.age+'<br><span style="white-space:nowrap;">邮箱</span>:'
            					+item.email+'<br>Tel:'
            					+item.phone+'</address></div><div class="clearfix"></div></div></div>').appendTo('#coachlist')
	            	})
            	}else{
            		// 失败
